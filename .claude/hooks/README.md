...
|---|---|---|---|
| [`guard-bash.sh`](guard-bash.sh) | PreToolUse | `Bash` | `--no-gpg-sign`, force-push to main/master, `reset --hard main`, `rm -rf .git`, `rm -rf .workingdir` |
| [`../../.config/agent/hooks/block_evasion.py`](../../.config/agent/hooks/block_evasion.py) | PreToolUse | `Bash` | praetor's hook-evasion interceptor: `--no-verify`, `git commit -n`, `LEFTHOOK=0`, `SKIP=…git`, `core.hooksPath=/dev/null`, `rm .git/hooks` (generated by `standardsctl adopt`; do not edit by hand) |
