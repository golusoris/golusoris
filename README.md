...
- **Step 9 — Auth + Authz**: `auth/jwt/` · `auth/apikey/` (HMAC-SHA256, pluggable store) · `auth/oidc/` (PKCE, go-oidc/v3) · `auth/session/` (server-side, pluggable store) · `auth/lockout/` (per-identity brute-force lockout) · `auth/captcha/` (Turnstile + hCaptcha + reCAPTCHA verifiers) · `auth/policy/` (zxcvbn + HIBP k-anonymity) · `auth/recovery/` (one-time recovery codes + reset tokens) · `auth/magiclink/` (passwordless email links) · `auth/linking/` (multi-IdP identity linking) · `auth/impersonate/` (audited admin-as-user with banner) · `auth/passkeys/` (WebAuthn + TOTP via go-webauthn + pquerna/otp) · `auth/oauth2server/` (minimal OAuth 2.1 issuer with PKCE) · `auth/scim/` (SCIM 2.0 user/group provisioning) · `authz/` (Casbin RBAC/ABAC)
...
- **Step 14 — Search + AI**: `search/` (Indexer/Searcher interface + MemorySearcher) · `ai/llm/` (Chat/Stream/Embed interface + OpenAI-compatible HTTP client) · `ai/vector/` (pgvector helpers: From, SimilaritySearch, RegisterTypes)
- **Step 15 — Commerce** (partial): `money/` (currency-aware minor-unit Money type, ISO 4217, zero-decimal support) · `payments/stripe/` (Checkout sessions, Customer Portal, Payment Intents via stripe-go/v82)
